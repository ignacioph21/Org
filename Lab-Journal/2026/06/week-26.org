...
** [12:08]
Correcciones para el espaciador y sensor:
- Pasante que agarra al plexiglas darle un poco más de tolerancia.
- Guía del espaciador más estreccha.
- Huella para el slag.

** [13:19]
Al final para el setup decidimos que los plexi simplemente encaastren en la caja del fonodo en ranuras de 0.5 cm de profundidad, y luego la base sea una grilla de agujeros donde pueden entrar los pegs de lo que querramos pegar al fondo, al estilo de un lego. Además las piezas del fondo se conectan entre sí mediante una juntura triangular tipo pieza de rompecabezas.

** [13:54]
Estuve recién analizando las PDFs de las series temporales. Para la que está justo después de la paleta del motor pareciera que es más angosta que una Gaussiana. Luego, la del sensor intermedio pareciera ser como la distribución de Tayfun de WWT y la del sensor luego del rompimiento tiene una asimetría bastante marcada. Una cosa importante es que en todos los caso hay un pico secundario en los valores positivos correspondiente a las patadas de intervalo ~15 s que daba el motor para generar ondas grandes.

* 2026-06-26

** [10:58]
Para instalar Autodesk Fusion en Linux Mint utilicé la versión de cryingfly en codeberg: [[https://codeberg.org/cryinkfly/Autodesk-Fusion-360-on-Linux][aquí]]. Y la que funcionó fue para =Open a terminal and run this command to install Autodesk Fusion with the Wine Z-window fix runtime=. La siguiente (=Open a terminal and run this command to install Autodesk Fusion with system Wine=) nunca abrió.

** [15:36]
Seguimos con el diseño del setup para el canal dentro de la cuba grande. Volvimos a la idea de tornillos de plástico ya que los bloques más grandes de la rampa al tener poca densidad harían que flote todo. Además la idea es tener hacia los laterales de las paredes del canal lugares donde colocar las pesas de hierro de forma tal que lastren toda la estructura, y en el caso de que tenga una única pared, que evite el torque de la misma.

** [15:38]
Mandamos a imprimir un tornillo M6x6 y una tuerca M6 para verificar que roscarían bien antes de seguir con el diseño.
